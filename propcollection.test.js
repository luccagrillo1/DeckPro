// Two independent prop collections ("DeckPro" / "DeckPro 2"): two decks on
...
(async () => {
  try {
    ...
    // Seed with a foreign user collection that must survive untouched.
    fs.writeFileSync(confPath, PropDocument.encode(PropDocument.fromObject({
      cues: [], propCollections: [{ uuid: { string: 'AAAAAAAA-0000-0000-0000-000000000009' }, name: 'My Own Props', items: [] }],
    })).finish());
    ...
    ok('each export reports which collection it wrote to',
      rEvent.propCollectionName === 'DeckPro' && rGathering.propCollectionName === 'DeckPro 2',
      JSON.stringify([rEvent.propCollectionName, rGathering.propCollectionName]));

    let conf = readConf();
    const c1 = collection(conf, 'DeckPro'), c2 = collection(conf, 'DeckPro 2');
    ok('both "DeckPro" and "DeckPro 2" collections exist', !!c1 && !!c2);
    ...
    ok('Event deck\'s prop lives in "DeckPro"', cueText(conf, b1).includes('EVENT_SENTINEL'));
    ok('Gathering deck\'s prop lives in "DeckPro 2"', cueText(conf, b2).includes('GATHERING_SENTINEL'));
    ...
    ok('Event presentation triggers "DeckPro" slots', evUuids.length > 0 && evUuids.every(u => b1.has(u)), evUuids.join(','));
    ok('Gathering presentation triggers "DeckPro 2" slots', gaUuids.length > 0 && gaUuids.every(u => b2.has(u)), gaUuids.join(','));

    // Re-exporting Event (edited) must leave Gathering's collection alone.
    await encode(deck('Event', 'EVENT_EDITED', 1));
    conf = readConf();
    ok('re-exported Event updates "DeckPro"', cueText(conf, b1).includes('EVENT_EDITED'));
    ok('re-exporting Event leaves "DeckPro 2" intact', cueText(conf, b2).includes('GATHERING_SENTINEL'));
    ok('still exactly one of each collection after re-export',
      conf.propCollections.filter(c => c.name === 'DeckPro').length === 1 &&
      conf.propCollections.filter(c => c.name === 'DeckPro 2').length === 1);
    ...
    ok('a deck with no propCollection set defaults to "DeckPro"', rLegacy.propCollectionName === 'DeckPro');
  } catch (e) {
